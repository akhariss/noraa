...
class Router
{
    ...
    {
        // 0. Backward Compatibility & Redirection (SK-01)
        // If someone uses ?gate=xxx, 301 redirect to the pretty URL version
        if (isset($_GET['gate']) && !isset($_GET['url'])) {
            $gate = $_GET['gate'];
            $params = $_GET;
            unset($params['gate']);
            
            // Use the url() helper we created
            $newUrl = url($gate, $params);
            header("HTTP/1.1 301 Moved Permanently");
            header("Location: $newUrl");
            exit;
        }

        // 1. Get raw URL from .htaccess (pretty URL) or fallback to home
        ...
        // 2. Path Isolation: Check if accessing via /office
        if (strpos($urlPath, 'office') === 0) {
            $isOfficeRequest = true;
            // Remove 'office' prefix to find the matching route
            $urlPath = substr($urlPath, 6);
            $urlPath = trim($urlPath, '/');
            
            // Default gate for just /office is dashboard
            if ($urlPath === '') {
                $urlPath = 'dashboard';
            }

            // Security: Set noindex headers for all /office paths
        ...
        // 4. Security Enforcement: Admin routes MUST be accessed via /office
        if ($options['auth'] && !$isOfficeRequest) {
            // Block access to admin pages from public path
            self::notFound();
            return;
        }
