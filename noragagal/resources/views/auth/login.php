...
<head>
    ...
    <link rel="stylesheet" href="<?= APP_URL ?>/public/assets/css/auth.css?v=<?= time() ?>">
</head>
    ...
    <div class="auth-container">
        <div class="auth-card">
            ...
            <form id="loginForm" class="auth-form" method="POST" action="<?= url('login') ?>">
                ...
                <?php
                // Show error message from session flash
                if (isset($_SESSION['login_error'])):
                    $errorMsg = $_SESSION['login_error'];
                    unset($_SESSION['login_error']);
                ?>
                <div class="auth-message error" style="display: block; background-color: #f8d7da; border: 1px solid #f5c6cb; color: #721c24; padding: 12px; border-radius: 4px; margin-bottom: 16px; font-size: 14px;">
            ...
            <div class="auth-footer">
                <p><a href="<?= url('home') ?>">← Kembali ke Homepage</a></p>
            </div>
        </div>
    </div>
