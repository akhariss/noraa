...
<div class="registrasi-detail">
            ...
            <div class="nora-detail-header">
                ...
                <div style="display: flex; align-items: center; gap: 8px;">
                    ...
                    <button type="button" id="btnTambahPembayaran" onclick="toggleFormBayar()" style="
                        background: var(--primary); color: var(--gold); border: none; cursor: pointer;
    ...
    <div id="negativeConfirmModal" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); z-index: 10001; align-items: center; justify-content: center;">
        <div style="background: var(--white); border-radius: 12px; padding: 24px; max-width: 420px; width: 95%; box-shadow: 0 10px 40px rgba(0,0,0,0.2);">
            ...
            <div style="display: flex; gap: 12px; justify-content: flex-end;">
                ...
                <button type="button" onclick="confirmNegativePayment()" style="background: var(--primary); color: var(--gold); padding: 10px 24px; border: none; border-radius: 6px; font-weight: 800; cursor: pointer; font-size: 13px; box-shadow: 0 4px 12px rgba(10,31,68,0.2);">Ya, Lanjutkan</button>
            </div>
        </div>
    </div>
    ...
    <div id="successModal" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); z-index: 10002; align-items: center; justify-content: center;">
        <div style="background: var(--white); border-radius: 12px; padding: 32px; max-width: 400px; width: 90%; text-align: center;">
            ...
            <button onclick="closeSuccessModal()" style="background: var(--primary); color: var(--gold); padding: 12px 32px; border: none; border-radius: 6px; font-weight: 600; cursor: pointer;">OK</button>
        </div>
    </div>
        ...
        <form id="editKlienForm">
            ...
            <div style="display: flex; gap: 10px; justify-content: flex-end;">
                ...
                <button type="submit" style="
                    background: var(--primary); color: var(--gold); padding: 8px 18px;
                    border: none; border-radius: 6px; font-size: 12px; font-weight: 700; cursor: pointer;
...
function renderPembayaranDetail(data) {
            ...
            <div style="background: #fff9f0; border: 1px solid var(--gold); border-radius: 6px; padding: 12px;">
                <form onsubmit="submitSetTagihan(event)">
                    <div style="display: flex; gap: 8px; align-items: flex-end;">
                        ...
                        <button type="submit" style="padding: 9px 16px; background: var(--primary); color: var(--gold); border: none; border-radius: 6px; font-size: 11px; font-weight: 800; cursor: pointer; white-space: nowrap; text-transform: uppercase; letter-spacing: 0.5px;">Set Tagihan</button>
                    </div>
                </form>
            </div>
        `;
    ...
    if (data.riwayat && data.riwayat.length > 0) {
        riwayatHtml = '<div style="margin-bottom: 12px;"><strong style="font-size: 12px;">Riwayat Pembayaran:</strong>';
        riwayatHtml += '<div style="margin-top: 6px; border-radius: 4px; overflow: hidden;">';
        riwayatHtml += '<table style="width: 100%; font-size: 12px; border-collapse: collapse;">';
        riwayatHtml += '<thead><tr style="background: var(--cream);">';
        riwayatHtml += '<th style="padding: 8px; text-align: left; border-bottom: 1px solid var(--border); font-size: 11px; text-transform: uppercase; font-weight: 700;">Tanggal</th>';
        riwayatHtml += '<th style="padding: 8px; text-align: right; border-bottom: 1px solid var(--border); font-size: 11px; text-transform: uppercase; font-weight: 700;">Nominal</th>';
        riwayatHtml += '<th style="padding: 8px; text-align: left; border-bottom: 1px solid var(--border); font-size: 11px; text-transform: uppercase; font-weight: 700;">Catatan</th>';
        riwayatHtml += '<th style="padding: 8px; text-align: left; border-bottom: 1px solid var(--border); font-size: 11px; text-transform: uppercase; font-weight: 700;">Oleh</th>';
        riwayatHtml += '</tr></thead><tbody style="background: var(--white);">';
        data.riwayat.forEach(r => {
            ...
            riwayatHtml += `<tr style="border-bottom: 1px solid var(--border);">
                <td style="padding: 8px; color: var(--text); font-size: 13px;">${formatTanggal(r.tanggal_bayar)}</td>
                <td style="padding: 8px; text-align: right; font-weight: 700; ${nominalClass}; font-size: 13px;">${nominalPrefix}Rp ${formatNumber(Math.abs(nominal))}</td>
                <td style="padding: 8px; color: var(--text-muted); font-size: 13px;">${r.catatan || '-'}</td>
                <td style="padding: 8px; font-size: 13px; color: var(--text);">${r.name || r.username || '-'}</td>
            </tr>`;
        });
        riwayatHtml += '</tbody></table></div></div>';
    } else {
        ...
        <div style="background: #fdfdfd; border: 1px solid #eee; border-radius: 8px; padding: 14px; margin-top: 16px;">
            <form onsubmit="submitBayar(event)">
                ...
                <div style="display: flex; justify-content: flex-end; gap: 8px;">
                    ...
                    <button type="submit" style="padding: 6px 16px; background: var(--primary); color: var(--gold); border: none; border-radius: 5px; font-size: 11px; font-weight: 800; cursor: pointer; display: flex; align-items: center; gap: 6px;">
...
<div id="waPopupDetail" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(15,31,40,0.85); z-index: 100000; align-items: center; justify-content: center; backdrop-filter: blur(8px);">
    <div style="background: #F7F4EF; border-radius: 30px; padding: 45px; max-width: 650px; width: 95%; text-align: center; box-shadow: 0 40px 100px rgba(0,0,0,0.5); border: 2.5px solid #B8964F; animation: atomicModalFadeIn 0.35s cubic-bezier(0.34, 1.56, 0.64, 1); position: relative;">
        ...
        <div style="text-align: left; margin-bottom: 20px; background: rgba(255,255,255,0.7); padding: 15px 25px; border-radius: 16px; border: 1.5px solid #EEE; display: flex; justify-content: space-between; align-items: center;">
            ...
            <span style="background: #1B3A4B; color: #B8964F; font-size: 9px; font-weight: 950; padding: 5px 12px; border-radius: 6px; text-transform: uppercase; letter-spacing: 0.5px;">Custom Edit Mode</span>
        </div>
        ...
        <div style="margin-top: 25px; display: flex; gap: 12px; justify-content: center;">
            ...
            <button type="button" onclick="confirmSendWaDetail()" style="background: #1B3A4B; color: #B8964F; padding: 12px 55px; border: none; border-radius: 50px; font-weight: 950; cursor: pointer; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; box-shadow: 0 10px 25px rgba(27,58,75,0.3); transition: 0.2s;">Kirim Notifikasi</button>
        </div>
    </div>
</div>
