...
<script>
    ...
    let currentAction = '<?= $role === 6 ? "review" : "tutup" ?>';
</script>
...
<div class="registrasi-detail">
    ...
    <div id="finalCard" class="detail-card" style="background: var(--white); border-radius: 12px; padding: 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.04); margin-bottom: 24px; border: 2px solid <?= $role === 6 ? '#2196f3' : 'var(--primary)' ?>; transition: all 0.3s ease;">
        <div id="actionMessage" style="display: none; padding: 12px; border-radius: 8px; margin-bottom: 15px; font-size: 13px; font-weight: 600; text-align: center;"></div>
...
<script>
    function switchAction(type) {
        ...
        if (type === 'tutup') {
            ...
            if (notesLabel) notesLabel.innerText = "📋 CATATAN PENUTUPAN ARSIP";
            if (hubNotes) hubNotes.value = DB_TEMPLATES.tutup;
            if (btnText) btnText.innerText = "Simpan Penutupan";
            ...
            if (notesLabel) notesLabel.innerText = "📋 CATATAN PENINJAUAN KEMBALI";
            if (hubNotes) hubNotes.value = DB_TEMPLATES.review;
            if (btnText) btnText.innerText = "Simpan Peninjauan";
            if (btnIcon) btnIcon.innerText = "🔄";
        }
    }
    ...
    function handleFinalSubmit() {
        ...
        const status = (currentAction === 'tutup') ? 'ditutup' : 'back_to_process';
        ...
        // Show feedback toast like in registrasi_detail.php
        if (messageDiv) {
        ...
        formData.append('target_status', status);
        formData.append('notes', notes);
        formData.append('csrf_token', '<?= generateCSRFToken() ?>');

        // Use global APP_URL for consistent routing
        ...
        .then(data => {
            if (data.success) {
                if (messageDiv) {
                    ...
                    messageDiv.textContent = 'Berhasil disimpan!';
                }
                setTimeout(() => {
                    location.reload();
                }, 800);
                ...
                if (messageDiv) {
                    ...
                    messageDiv.textContent = 'Eror: ' + data.message;
                }
                btn.disabled = false;
                btn.style.opacity = '1';
    ...
    function sendWhatsAppNotificationFinal() {
        ...
        const waUrl = `https://wa.me/${REG_DATA.klien_hp}?text=${encodeURIComponent(message)}`;
