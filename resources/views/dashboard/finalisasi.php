...
<div class="finalisasi-container">
    ...
    <div class="dashboard-card" style="padding: 16px; margin-bottom: 16px;">
        <form method="GET" action="" class="nora-filter-toolbar">
            ...
            <input type="hidden" name="order" value="<?= htmlspecialchars($order ?? 'baru') ?>">
            ...
            <div class="refinement-row">
                <?php
                    $currentOrder = $order ?? 'baru';
                    $nextOrder = $currentOrder === 'baru' ? 'lama' : 'baru';
                    $sortIcon = $currentOrder === 'baru' ? '▲' : '▼';
                    $sortLabel = $currentOrder === 'baru' ? 'Terbaru' : 'Terlama';
                ?>
                <a href="?gate=finalisasi&filter=<?= urlencode($result['filter']) ?>&search=<?= urlencode($search) ?>&order=<?= $nextOrder ?>&page=1" class="sort-toggle active" title="Toggle urutan waktu penyelesaian ke <?= $sortLabel ?>" aria-label="Urutkan berdasarkan waktu penyelesaian <?= $sortLabel ?>">
                    <span><?= $sortIcon ?></span>
                </a>
                <a href="?gate=finalisasi&filter=all&search=<?= urlencode($search) ?>&order=<?= urlencode($order ?? 'baru') ?>&page=1" class="filter-tab all <?= $result['filter'] === 'all' ? 'active' : '' ?>">📦 Semua</a>
                <a href="?gate=finalisasi&filter=selesai&search=<?= urlencode($search) ?>&order=<?= urlencode($order ?? 'baru') ?>&page=1" class="filter-tab selesai <?= $result['filter'] === 'selesai' ? 'active' : '' ?>">✅ Diserahkan</a>
                <a href="?gate=finalisasi&filter=batal&search=<?= urlencode($search) ?>&order=<?= urlencode($order ?? 'baru') ?>&page=1" class="filter-tab batal <?= $result['filter'] === 'batal' ? 'active' : '' ?>">❌ Batal</a>
                <a href="?gate=finalisasi&filter=ditutup&search=<?= urlencode($search) ?>&order=<?= urlencode($order ?? 'baru') ?>&page=1" class="filter-tab ditutup <?= $result['filter'] === 'ditutup' ? 'active' : '' ?>">📂 Ditutup</a>
