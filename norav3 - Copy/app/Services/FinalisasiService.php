...
class FinalisasiService
{
    ...
    /**
     * Generic behavior-based list (for Penyerahan, Arsip, etc.)
     * @param array $behaviors Array of behavior_role values to filter
     */
    public function getBehaviorList(int $page, int $perPage, array $behaviors, string $search = '', string $order = 'baru', string $layanan = '', string $periode = ''): array
    {
        $offset = ($page - 1) * $perPage;
        
        $behaviorPlaceholders = implode(', ', $behaviors);
        $conditions = ["w.behavior_role IN ($behaviorPlaceholders)"];
        $params = [];
        ...
        try {
            ...
            Logger::error('getBehaviorList error', ['error' => $e->getMessage()]);
            throw $e;
        }
    }
