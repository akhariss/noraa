...
class ReportService
{
    ...
    public function getSummary(string $startDate, string $endDate): array
    {
        try {
            ...
            // 2. Registrasi Ditutup (Behavior 4, 5, 6, 7)
            $stmt = $conn->prepare("
                SELECT COUNT(*) as total 
                FROM registrasi r
                JOIN workflow_steps ws ON ws.id = r.current_step_id
                WHERE ws.behavior_role IN (4, 5, 6, 7)
                  AND r.selesai_batal_at BETWEEN ? AND ?
            ");
            $stmt->execute([$startDate . ' 00:00:00', $endDate . ' 23:59:59']);
    ...
    public function getMatrixTimeline(string $startDate, string $endDate): array
    {
        try {
            ...
            // Get all workflow steps for columns
            ...
            // Get all registrations that have activity (history) in this period
            $stmt = $conn->prepare("
                SELECT DISTINCT r.id, r.nomor_registrasi, k.nama as klien_nama, l.nama_layanan, 
                       r.current_step_id, r.target_completion_at, ws.behavior_role, r.created_at
                FROM registrasi r
                JOIN klien k ON k.id = r.klien_id
                JOIN layanan l ON l.id = r.layanan_id
                JOIN workflow_steps ws ON ws.id = r.current_step_id
                JOIN registrasi_history rh ON rh.registrasi_id = r.id
                WHERE (r.created_at BETWEEN ? AND ?) OR (rh.created_at BETWEEN ? AND ?)
                ORDER BY r.created_at DESC
            ");
            $stmt->execute([$range[0], $range[1], $range[0], $range[1]]);
            ...
            foreach ($registrasi as $reg) {
                ...
                for ($i = 0; $i < count($history); $i++) {
                    ...
                    if ($i + 1 < count($history)) {
                        ...
                        $end = in_array($behavior, [5, 6, 7]) ? $start : time();
                    }
