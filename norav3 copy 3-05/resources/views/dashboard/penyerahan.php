<?php
/**
 * Penyerahan View - Behavior 4 only
 * Search only, finalisasi-style table
 */

$currentUser = getCurrentUser();
$pageTitle = 'Penyerahan Berkas';
$activePage = 'penyerahan';
...
<div class="hero-authority">
    <span class="hero-badge-auth">Penyerahan Berkas</span>
    <p class="hero-p-auth">Berkas yang siap diserahkan kepada klien</p>
</div>

<!-- Command Area -->
<div class="action-vault-v5">
    <form method="GET" action="" id="penyerahanForm">
        <input type="hidden" name="gate" value="penyerahan">
        ...
        <div class="f-row-v5">
            ...
            <button type="button" onclick="window.location.href='?gate=penyerahan'" style="padding:10px; background:#fff; border:1px solid #eee; border-radius:10px; cursor:pointer;" title="Reset">
    ...
    <table style="width: 100%; border-collapse: collapse;">
        ...
        <tbody>
            <?php if (empty($result['data'])): ?>
                <tr><td colspan="6" style="padding: 60px; text-align: center; color: #ccc; font-weight: 600;">Belum ada berkas yang siap diserahkan.</td></tr>
            <?php else: ?>
                ...
                <tr class="lx-row">
                    ...
                    <td style="text-align: center;">
                        <a href="<?= APP_URL ?>/index.php?gate=registrasi_detail_finalisasi&id=<?= $p['id'] ?>&from=penyerahan" class="btn-auth-detail"><i>o</i> Detail</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            <?php endif; ?>
...
<div style="display: flex; flex-direction: column; align-items: center; gap: 10px; margin-top: 30px; padding-bottom: 20px; font-family: 'DM Sans', sans-serif;">
    <div style="display: flex; gap: 6px; align-items: center;">
        <?php 
        $baseQuery = http_build_query(['gate' => 'penyerahan', 'order' => $order, 'search' => $search]);
        $p = $result['pagination'];
...
<script>
let searchTimer;
function debounceSearch() { clearTimeout(searchTimer); searchTimer = setTimeout(() => { document.getElementById('penyerahanForm').submit(); }, 800); }
function toggleSort() {
    const cur = document.getElementById('orderInput').value;
    document.getElementById('orderInput').value = (cur === 'baru') ? 'lama' : 'baru';
    document.getElementById('penyerahanForm').submit();
}
</script>
