...
class Auth
{
    ...
    public static function startSecureSession(): void
    {
        ...
        ini_set('session.cookie_lifetime', '0'); // Browser session length
        ...
        if (self::isLoggedIn()) {
            $gate = $_GET['gate'] ?? 'home';
            $publicGates = ['home', 'lacak', 'detail', 'verify_tracking', 'health', 'login'];
            $isPublicGate = in_array($gate, $publicGates, true);
            ...
            if (!$isPublicGate) {
                ...
                if (time() - $lastActivity > self::SESSION_TIMEOUT) {
                    ...
                    if (self::isAjax()) {
                        ...
                        header('Location: ' . APP_URL . '/index.php?gate=login&timeout=1');
                    }
                    exit;
                }
                ...
                // Validate fingerprint (anti-hijacking)
    ...
    {
        return hash('sha256', (string)$userId . $role . session_id());
    }
