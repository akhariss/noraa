...
class Registrasi
{
    ...
    public function findById(int $id): ?array
    {
        ...
        if ($this->columnExists('kendala_flag')) {
            $selectCols .= " p.kendala_flag,";
        } else {
            // Logic M-07: Get the latest flag status from history if column doesn't exist yet
            $selectCols .= " (SELECT h.flag_kendala_active FROM registrasi_history h 
                              WHERE h.registrasi_id = p.id 
                              ORDER BY h.created_at DESC, h.id DESC LIMIT 1) AS kendala_flag,";
        }
