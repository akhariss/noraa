<?php
/**
 * Update Status View - Fully Synced with Registrasi Filter & Table System
 * Ensures consistent filtering and data presentation across modules.
 */

$currentUser = getCurrentUser();
$pageTitle = 'Update Status';
$activePage = 'update_status_list';
...
<div class="hero-authority">
    <span class="hero-badge-auth">Update Status Aktif</span>
    <p class="hero-p-auth">Kelola dan perbarui status registrasi yang sedang dalam proses pengerjaan</p>
</div>

<!-- Command Center -->
<div class="action-vault-v4">
    <form method="GET" action="" id="v4Form">
        <input type="hidden" name="gate" value="update_status_list">
        <input type="hidden" name="tab" id="tabH4" value="aktif">
        ...
        <div class="filter-full-row">
            ...
            <div class="f-item">
                ...
                <select name="status" id="statusSel" onchange="this.form.submit()" class="f-select-v4">
                    ...
                    <?php foreach ($allSteps as $s): ?>
                        <?php if(in_array((int)$s['behavior_role'], [0, 1, 2, 3])): ?>
            ...
            <div class="f-item">
                <button type="button" onclick="window.location.href='?gate=update_status_list'" class="btn-reset-v4" title="Reset Semua Filter">
...
<div class="table-vault-v4">
    <div class="table-head-v4">
        <h4 style="font-size:12px; font-weight:900; color:var(--primary); margin:0; text-transform:uppercase;">DAFTAR REGISTRASI AKTIF (<?= $displayTotal ?>)</h4>
            ...
            <tbody>
                ...
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td style="text-align:center;">
                            <a href="?gate=registrasi_detail&id=<?= $p['id'] ?>&from=update_status_list" class="btn-auth-detail"><i>o</i> Detail</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
...
<div style="display: flex; flex-direction: column; align-items: center; gap: 10px; margin-top: 30px; padding-bottom: 20px; font-family: 'DM Sans', sans-serif;">
    <div style="display: flex; gap: 6px; align-items: center;">
        ...
        <?php if ($page > 1): ?>
            <a href="?<?= $baseQuery ?>&page=<?= $page - 1 ?>" style="padding: 8px 14px; background: #fff; border: 1px solid #ddd; border-radius: 8px; text-decoration: none; color: var(--primary); font-size: 13px; font-weight: 700;">&laquo; Prev</a>
        <?php endif; ?>
        ...
        <?php if ($page < $totalPages): ?>
            <a href="?<?= $baseQuery ?>&page=<?= $page + 1 ?>" style="padding: 8px 14px; background: #fff; border: 1px solid #ddd; border-radius: 8px; text-decoration: none; color: var(--primary); font-size: 13px; font-weight: 700;">Next &raquo;</a>
        <?php endif; ?>
    </div>
</div>
<?php endif; ?>
...
async function v4Export() {
    ...
    try {
        ...
        let csv = "\uFEFF";
        let meta = ['DAFTAR REGISTRASI AKTIF','Download: '+new Date().toLocaleString(),'Total: ' + result.total];
        ...
        link.download = 'Update_Status_Full_' + new Date().toISOString().split('T')[0] + '.csv';
        link.click();
    } catch (err) {
