<?php
/**
 * Tentang and CTA Partial (Final Production)
 * Support optional photo from database
 */
...
$tentang = [
    ...
    'photo'      => cmsContent($homepageData, 'tentang', 'photo', ''), // Optional photo URL
    'benefits'   => cmsItems($homepageData, 'tentang', 'benefit'),
];

$ctaSec = cmsSection($homepageData, 'cta');
?>
            ...
            <div class="tentang-img">
                <?php if (!empty($tentang['photo'])): ?>
                    ...
                    <div class="tentang-photo">
                        <img src="<?= htmlspecialchars($tentang['photo']) ?>" alt="<?= htmlspecialchars($tentang['name']) ?>">
                    </div>
                    <div class="tentang-photo-info">
                        <div class="tentang-img-name"><?= htmlspecialchars($tentang['name']) ?></div>
                        <div class="tentang-img-title"><?= htmlspecialchars($tentang['role']) ?></div>
                    </div>
                <?php else: ?>
                    <!-- Icon mode (default) -->
                ...
                <div class="tentang-badge">
                    <strong><?= htmlspecialchars($tentang['experience']) ?></strong>
                    <span>Tahun Pengalaman</span>
                </div>
            </div>
            <div class="tentang-content">
                <h2><?= htmlspecialchars($tentang['title']) ?></h2>
                <div class="tentang-narasi">
                    <?= htmlspecialchars($tentang['quote']) ?>
                </div>
                <ul class="tentang-list">
                    <?php foreach ($tentang['benefits'] as $b): ?>
                    <li><?= $checkSvg ?> <?= htmlspecialchars($b['title'] ?? '') ?></li>
                    <?php endforeach; ?>
                </ul>
            </div>
        </div>
...
<section class="section tracking" id="track">
    <div class="container">
        <div class="tracking-wrapper">
            ...
            <a href="<?= APP_URL ?>/index.php?gate=lacak" class="btn-track" aria-label="Lacak Registrasi Anda">
...
<section class="cta" id="kontak">
    <div class="container">
        <div class="cta-box">
            ...
            <a href="https://wa.me/<?= sanitizePhoneForWa($footerPhone) ?>" class="cta-btn" target="_blank" aria-label="Konsultasi Kebutuhan Hukum via WhatsApp (Footer)">
