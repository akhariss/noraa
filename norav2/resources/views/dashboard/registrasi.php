...
.btn-reset-v4:hover { transform: rotate(90deg); color: var(--gold); background: var(--primary); }
...
.lx-auth-table td { padding: 14px 20px; font-size: 13.5px; color: var(--primary); border-bottom: 1px solid #f1f1f1; vertical-align: middle; }
...
<div class="action-vault-v4">
    <form method="GET" action="" id="v4Form">
        ...
        <input type="hidden" name="tab" id="tabH4" value="<?= htmlspecialchars($filterTab ?? 'aktif') ?>">

        <!-- Tabs Section -->
        <div class="tabs-organic">
            <a href="javascript:void(0)" onclick="setTab4('aktif')" class="tab-v4 <?= ($filterTab ?? 'aktif') === 'aktif' ? 'active' : '' ?>">AKTIF (PROSES)</a>
            <a href="javascript:void(0)" onclick="setTab4('arsip')" class="tab-v4 <?= ($filterTab ?? '') === 'arsip' ? 'active' : '' ?>">ARSIP / SELESAI</a>
            <a href="javascript:void(0)" onclick="setTab4('semua')" class="tab-v4 <?= ($filterTab ?? '') === 'semua' ? 'active' : '' ?>">SEMUA HISTORI</a>
        </div>
...
function v4Export() {
    const table = document.getElementById('v4Table'); if (!table) return;
    let csv = "\uFEFF";
    let meta = ['DAFTAR REGISTRASI SRI ANAH','Download: '+new Date().toLocaleString(),'Total: <?= $displayTotal ?>'];
    ...
    table.querySelectorAll('tr').forEach(tr => {
        ...
        tr.querySelectorAll('th, td').forEach((c, idx) => {
            ...
            if (idx === 6 && c.tagName === 'TD') txt = c.querySelector('div').innerText + ' ' + c.querySelectorAll('div')[1].innerText;
    ...
    link.download = 'Registrasi_AA_v5.0_Full_' + new Date().toISOString().split('T')[0] + '.csv';
    link.click();
}
</script>
