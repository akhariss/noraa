...
// 2. Edit Klien Modal Logic
function openEditModal() { document.getElementById('editModal').style.display = 'flex'; }
function closeEditModal() { document.getElementById('editModal').style.display = 'none'; }

const editKlienForm = document.getElementById('editKlienForm');
if (editKlienForm) {
    editKlienForm.addEventListener('submit', function(e) {
        e.preventDefault();
        const formData = new FormData(this);
        const messageDiv = document.getElementById('actionMessage');
        ...
        fetch(APP_URL + '/index.php?gate=update_registrasi_klien', {
            method: 'POST',
            body: formData
        })
        .then(response => response.json())
        .then(data => {
            ...
            if (messageDiv) {
                ...
                else if(submitBtn) { submitBtn.disabled = false; submitBtn.textContent = 'Simpan Perubahan'; }
            }
        });
    });
}
...
function autoFillCatatan() {
    ...
    if (selectedStatus && statusTemplates[selectedStatus]) {
        ...
        // Smart Replacement using Global REG_DATA
        note = note.replace(/\{nama_klien\}/g, REG_DATA.nama || '')
                   .replace(/\{nomor_registrasi\}/g, REG_DATA.nomor || '')
                   .replace(/\{status\}/g, statusLabel)
                   .replace(/\{user_name\}/g, REG_DATA.sender || 'Admin')
                   .replace(/\{nama_pengirim\}/g, REG_DATA.sender || 'Admin')
                   .replace(/\{nama_kantor\}/g, REG_DATA.kantor || 'Kantor Notaris')
                   .replace(/\{phone\}/g, REG_DATA.hp || '')
                   .replace(/\{tanggal\}/g, tanggalReal || REG_DATA.tanggal);
