...
class CMSEditorService
{
    ...
    public function getPageForEditing(string $pageKey): array
    {
        ...
        $sections = $this->sectionModel->getByPageId((int)$page['id']);
        $result = ['page' => $page, 'sections' => []];

        foreach ($sections as $section) {
            $contentRows = $this->contentModel->getBySectionId((int)$section['id']);
            $itemRows = $this->itemModel->getBySectionId((int)$section['id']);
    ...
    public function updateContent(int $contentId, string $newValue): array
    {
        try {
            ...
            // e.g., "&amp;amp;" → "&amp;" → "&"
            $cleanValue = html_entity_decode($newValue, ENT_QUOTES | ENT_HTML5, 'UTF-8');
            $updated = $this->contentModel->update($contentId, $cleanValue);
            return $updated
