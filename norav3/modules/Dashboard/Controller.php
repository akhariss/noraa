...
class Controller
{
    ...
    public function showRegistrasiDetailFinalisasi(int $id): void
    {
        ...
        // Detail finalisasi boleh diakses untuk status non-aktif:
        // Selesai/Penyerahan (4), Diserahkan (5), Ditutup (6), Batal (7), Review (8)
        if (!in_array((int)($registrasi['behavior_role'] ?? 0), [4, 5, 6, 7, 8], true)) {
            header('Location: ' . APP_URL . '/index.php?gate=registrasi');
            exit;
        }
    ...
    public function registrasi(): void
    {
        ...
        $filterTab = isset($_GET['tab']) ? trim($_GET['tab']) : 'semua';
        ...
        // Validasi tab mode
        if (!in_array($filterTab, ['aktif', 'arsip', 'semua', 'review', 'penyerahan'])) {
            $filterTab = 'semua';
        }
        // Validasi bayar mode
        ...
        // Handle AJAX Export v6.15 (Fetch all filtered data)
        if (isset($_GET['ajax_export']) && $_GET['ajax_export'] === '1') {
            header('Content-Type: application/json');
            $allData = $this->registrasiModel->getWithFilters(
                $search, $filterStatus, $filterLayanan, 1000, 0, $filterOrder, $filterTab, $filterBayar, $filterPeriode
            );
            
            // Sync flags for export
            $exportResult = [];
            foreach ($allData as $p) {
                ...
                $exportResult[] = array_merge($p, ['has_flag' => $hasFlag ? 'YA' : 'TIDAK']);
            }
            
            echo json_encode(['data' => $exportResult, 'total' => count($exportResult)]);
            exit;
        }
    ...
    /**
     * Update Status List - Shows only active registrations (behavior 0,1,2,3)
     * No smart filter, search only
     */
    public function updateStatusList(): void
    {
        $page = isset($_GET['page']) ? max(1, (int)$_GET['page']) : 1;
        $perPage = 20;
        $search = isset($_GET['search']) ? trim($_GET['search']) : '';
        ...
        $filterLayanan = isset($_GET['layanan']) ? trim($_GET['layanan']) : '';
        $filterOrder = isset($_GET['order']) ? trim($_GET['order']) : 'terbaru';
        $filterTab = 'aktif'; // Locked for Update Status view
        $filterBayar = isset($_GET['bayar']) ? trim($_GET['bayar']) : '';
        $filterPeriode = isset($_GET['periode']) ? trim($_GET['periode']) : '';
        $filterFlag = isset($_GET['flag']) ? trim($_GET['flag']) : '';
        ...
        $registrasiPage = $this->registrasiModel->getWithFilters(
            $search, $filterStatus, $filterLayanan, $perPage, $offset, $filterOrder, $filterTab, $filterBayar, $filterPeriode
        );
        ...
        foreach ($registrasiPage as $p) {
            try {
                ...
                if ($filterFlag !== '') {
                    if ($filterFlag === '1' && !$hasFlag) continue;
                    if ($filterFlag === '0' && $hasFlag) continue;
                }
            } catch (Exception $e) {
                $hasFlag = false;
                if ($filterFlag === '1') continue;
            }
            $registrasiWithFlags[] = array_merge($p, ['has_flag' => $hasFlag]);
        }
        ...
        require VIEWS_PATH . '/dashboard/update_status.php';
    }

    /**
     * Penyerahan page - Shows behavior 4 only
     */
    public function penyerahan(): void
    {
        if (!verifyUserRole(ROLE_OWNER)) {
            showErrorPage(403, 'Akses Ditolak', 'Hanya Administrator yang dapat mengakses halaman penyerahan.');
        }
        ...
        if (!in_array($order, ['baru', 'lama'], true)) $order = 'baru';

        $finalisasiService = new \App\Services\FinalisasiService();
        $result = $finalisasiService->getBehaviorList($page, $perPage, [4], $search, $order);

        $currentUser = getCurrentUser();

        require VIEWS_PATH . '/dashboard/penyerahan.php';
    }

    /**
     * Arsip page - Shows behavior 5,6
     * Tabs: semua/diserahkan/batal, filters: urutan/layanan/periode
     */
    public function arsip(): void
    {
        ...
        if (!in_array($order, ['baru', 'lama'], true)) $order = 'baru';
        if (!in_array($arsipTab, ['semua', 'diserahkan', 'batal'], true)) $arsipTab = 'semua';

        // Map arsip_tab to behaviors
        $behaviors = match ($arsipTab) {
            'diserahkan' => [5],
            'batal' => [6],
            default => [5, 6],
        };
        ...
        $finalisasiService = new \App\Services\FinalisasiService();
        $result = $finalisasiService->getBehaviorList($page, $perPage, $behaviors, $search, $order, $filterLayanan, $filterPeriode);

        $layanan = $this->layananModel->getAll();
        $currentUser = getCurrentUser();

        require VIEWS_PATH . '/dashboard/arsip.php';
    }

    /**
     * App Settings - Grid with User Mgmt, CMS, Backup, Audit
     */
    public function appSettings(): void
    {
        requireRole(ROLE_OWNER);
        $currentUser = getCurrentUser();
        require VIEWS_PATH . '/dashboard/app_settings.php';
    }
    ...
    public function laporan(): void
    {
        ...
        require VIEWS_PATH . '/dashboard/reports/index.php';
    }

    /**
     * Laporan Registrasi & Matrix
     * Refactored: Business logic moved to ReportService. View receives pre-computed data.
     */
    public function laporanRegistrasi(): void
    {
        requireRole(ROLE_OWNER);
        $currentUser = getCurrentUser();
        $activePage = 'laporan';
        ...
        // Get raw data (with fallback defaults for robustness)
        $summary = $this->reportService->getSummary($startDate, $endDate) ?: [
            'registrasi_baru' => 0,
            'registrasi_ditutup' => 0,
            'total_tagihan' => 0,
            'total_terbayar' => 0
        ];
        $matrix = $this->reportService->getMatrixTimeline($startDate, $endDate);
        
        // Pre-compute all display metrics (logic centralized in service/controller)
        $totalMasuk = count($matrix['berjalan']) + count($matrix['selesai']);
        $selesaiCount = count($matrix['selesai']);
        
        // Get complete distribution (includes all services, even zero-count)
        $allDistribution = $this->reportService->getCompleteServiceDistribution($startDate, $endDate);
        
        // Compute stage counts
        $allStageCounts = $this->reportService->getStageCounts($matrix['steps_selesai'], $matrix['berjalan'], $matrix['selesai']);
        
        // Count overdue items using EXCLUDED_ROLES from config
        $totalOverdue = $this->reportService->countOverdue($matrix['steps_selesai'], array_merge($matrix['berjalan'], $matrix['selesai']));
        ...
        $piutang = ($summary['total_tagihan'] ?? 0) - ($summary['total_terbayar'] ?? 0);
        
        // Fetch CMS branding with fallback to OFFICE_* constants
        $cmsBranding = $this->reportService->getCMSBranding();

        if (isset($_GET['format']) && $_GET['format'] === 'download') {
            ob_start();
            require VIEWS_PATH . '/dashboard/reports/registrasi_print.php';
            $html = ob_get_clean();
            ...
            $filename = "Laporan_Registrasi_" . date('Ymd', strtotime($startDate)) . "_to_" . date('Ymd', strtotime($endDate)) . ".pdf";
            $dompdf->stream($filename, ["Attachment" => true]);
            exit;
        } elseif (isset($_GET['format']) && $_GET['format'] === 'print') {
            require VIEWS_PATH . '/dashboard/reports/registrasi_print.php';
        } else {
            require VIEWS_PATH . '/dashboard/reports/registrasi.php';
        }
    }
     ...
     * Laporan Keuangan
     */
    public function laporanKeuangan(): void
    {
        requireRole(ROLE_OWNER);
        $currentUser = getCurrentUser();
        $activePage = 'laporan';
        ...
        // Detailed unpaid/paid active
        $conn = \App\Adapters\Database::getInstance();
        $activeUnpaid = $conn->prepare("
            SELECT r.nomor_registrasi, k.nama as klien_nama, l.nama_layanan, t.total_tagihan, t.jumlah_bayar, (t.total_tagihan - t.jumlah_bayar) as sisa
            FROM registrasi r
            JOIN klien k ON k.id = r.klien_id
            JOIN layanan l ON l.id = r.layanan_id
            JOIN transaksi t ON t.registrasi_id = r.id
            JOIN workflow_steps ws ON ws.id = r.current_step_id
            WHERE ws.behavior_role NOT IN (4, 5, 6, 7)
              AND t.jumlah_bayar < t.total_tagihan
            ORDER BY sisa DESC
        ");
        $activeUnpaid->execute();
        $unpaidList = $activeUnpaid->fetchAll();

        $activePaid = $conn->prepare("
            SELECT r.nomor_registrasi, k.nama as klien_nama, l.nama_layanan, t.total_tagihan, t.jumlah_bayar, ws.label as status_label
            FROM registrasi r
            JOIN klien k ON k.id = r.klien_id
            JOIN layanan l ON l.id = r.layanan_id
            JOIN transaksi t ON t.registrasi_id = r.id
            JOIN workflow_steps ws ON ws.id = r.current_step_id
            WHERE ws.behavior_role NOT IN (4, 5, 6, 7)
              AND t.jumlah_bayar >= t.total_tagihan AND t.total_tagihan > 0
            ORDER BY r.updated_at DESC
        ");
        $activePaid->execute();
        $paidList = $activePaid->fetchAll();

        // Transaction Timeline
        $historyStmt = $conn->prepare("
            SELECT th.created_at, r.nomor_registrasi, k.nama as klien_nama, th.nominal_bayar, th.catatan, u.name as oleh
        ...
        ");
        $historyStmt->execute([$startDate . ' 00:00:00', $endDate . ' 23:59:59']);
        $history = $historyStmt->fetchAll();

        if (isset($_GET['format']) && $_GET['format'] === 'download') {
            ob_start();
            require VIEWS_PATH . '/dashboard/reports/keuangan_print.php';
            $html = ob_get_clean();
            ...
            $filename = "Laporan_Keuangan_" . date('Ymd', strtotime($startDate)) . "_to_" . date('Ymd', strtotime($endDate)) . ".pdf";
            $dompdf->stream($filename, ["Attachment" => true]);
            exit;
        } elseif (isset($_GET['format']) && $_GET['format'] === 'print') {
            require VIEWS_PATH . '/dashboard/reports/keuangan_print.php';
        } else {
            require VIEWS_PATH . '/dashboard/reports/keuangan.php';
        }
    }

    /**
     * Laporan Aktivitas Tim
     */
    public function laporanAktivitas(): void
    {
        requireRole(ROLE_OWNER);
        $currentUser = getCurrentUser();
        $activePage = 'laporan';
        ...
        $rankings = $this->reportService->getUserRankings($startDate, $endDate);

        if (isset($_GET['format']) && $_GET['format'] === 'print') {
            require VIEWS_PATH . '/dashboard/reports/aktivitas_print.php';
        } else {
            require VIEWS_PATH . '/dashboard/reports/aktivitas.php';
        }
    }
}
